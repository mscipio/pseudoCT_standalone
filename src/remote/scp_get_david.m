function ssh2_struct = scp_get_david(ssh2_struct, remoteFilename, localPath, remotePath)
...
if nargin < 2
    ...
    if ~strcmp(ssh2_struct.hostname , '127.0.0.1')
        ...
        for ii=1:length(remoteFilename)
            system(sprintf('cp %s %s', fullfile(remotePath, remoteFilename{ii}), fullfile(localPath)));
        end
    end
end