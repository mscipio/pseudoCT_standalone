function [pass,accepted] = uipassword(str,lbl,prompt)
...
% pass = uipassword(str,lbl,prompt)  -visible prompt (default:'Password:')
...
if nargin<3 || isempty(prompt), prompt = 'Password:'; end %visible prompt
%checks
if ~usejava('awt') %matlab was started with -nojvm flag (ie no java) 
    disp(lbl)
    pass = input(prompt,'s'); %revert to simple command line queries
    accepted = true;
    return
end
%main
ScreenSize = get(0,'ScreenSize');
hFig  = figure('WindowStyle','modal','Units','pixels','Position',[(ScreenSize(3:4)-[300 95])/2 300 95],'Name',lbl,'Resize','off','NumberTitle','off','Menubar','none','Color',[0.9 0.9 0.9],'KeyPressFcn',@KeyPress,'CloseRequestFcn',@(~,~)uiresume);
uicontrol(hFig,'Style','text','Units','pixels','Position',[20 55 260 20],'String',prompt,'HorizontalAlignment','left','FontSize',10,'BackGroundColor',[0.9 0.9 0.9]);
hPass = uicontrol(hFig,'Style','text','Units','pixels','Position',[20 25 260 20],'FontSize',10,'BackGroundColor','w');
pass = ''; %init
accepted = false;
