...
%    Version 2.0
%
% Dec/17/2017: Version to allow connection to the local host: 127.0.0.1;
...
if nargin < 2
    ...
    if ~strcmp(ssh2_struct.hostname , '127.0.0.1')
        ...
        if isempty(remoteFilename)
            for ii=1:length(localFilename)
                system(sprintf('cp %s %s', fullfile(localPath, localFilename{ii}), remotePath));
            end
        else
            for ii=1:length(localFilename)
                system(sprintf('cp %s %s', fullfile(localPath, localFilename{ii}), fullfile(remotePath, remoteFilename{ii})));
            end
        end
    end
end
